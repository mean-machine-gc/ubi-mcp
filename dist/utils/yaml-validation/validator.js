...
import { MCPConsistencyValidator } from "./validator-consistency";
import { MCPValidator } from "./validator-core";
// Tool: Validate single YAML content
export async function validateYAMLContent(content, fileType) {
...
export async function validateAggregateConsistency(files) {
...
export async function quickValidationCheck(content, fileType) {
    const validator = new MCPValidator();
    const result = validator.validateYAMLContent(content, fileType);
...
export async function extractAggregateStates(lifecycleContent) {
    const validator = new MCPValidator();
...
function generateConsistencySummary(result) {
    const issues = [];
...
function generateQuickFeedback(result) {
    if (result.isValid) {
        ...
        const criticalCount = result.errors.filter((e) => e.severity === 'error').length;
