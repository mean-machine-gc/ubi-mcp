...
export const InvariantSetSchema = Type.Record(Type.String({ pattern: '^[a-z][a-z_]*$' }), Type.String({ minLength: 5 }));
export const TransitionsSchema = Type.Record(Type.String({ pattern: '^[a-z_]+_to_[a-z_]+$' }), Type.Array(Type.String()));
// Extract states from a YAML content string
export function extractStatesFromYAML(yamlContent) {
    try {
...
export function createDynamicInvariantsSchema(states) {
    const properties = {
...
export function createLifecycleSchema(yamlContent) {
