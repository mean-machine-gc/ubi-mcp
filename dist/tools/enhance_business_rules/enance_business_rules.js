...
import { z } from 'zod';
export const addEnhanceBusinessRules = (mcp) => (session) => {
    mcp.addTool({
        ...
        execute: async (args) => {
            const { current_lifecycle, focus_areas = ["security", "edge_cases", "compliance"], domain_context = "" } = args;
...
const sampleAnalysis = (sample) => (current_lifecycle) => async (domain_context) => {
...
const sampleByFocusArea = (sample) => (focus_areas) => (currentAnalysis) => (domain_context) => async (current_lifecycle) => {
    const securityEnhancement = focus_areas.includes("security") ? await sample(`Analyze the security aspects of this lifecycle:
      ...
      Be specific and actionable.`) : "Security analysis skipped.";
    const edgeCaseAnalysis = focus_areas.includes("edge_cases") ? await sample(`Identify missing edge cases and failure scenarios:
      ...
      Focus on realistic, business-impacting scenarios.`) : "Edge case analysis skipped.";
    const complianceAnalysis = focus_areas.includes("compliance") ? await sample(`Analyze compliance and regulatory considerations:
      ...
      Be specific about regulatory requirements.`) : "Compliance analysis skipped.";
    const missingRulesAnalysis = await sample(`Identify missing business rules and operations:
    ...
    const improvements = await sample(`Create concrete improvement recommendations:
