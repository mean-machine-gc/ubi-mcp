...
import { z } from 'zod';
export const addGenerateTypes = (mcp) => (session) => {
    mcp.addTool({
        name: "generate_types",
        ...
        execute: async (args) => {
            const { lifecycle_yaml, assertions_yaml = "", type_philosophy = "strict", include_validation_patterns = true } = args;
...
const sampleBaseTypes = (sample) => (typeRequirements) => async (type_philosophy) => {
...
const sampleMessageTypes = (sample) => (typeRequirements) => (stateTypes) => async (include_validation_patterns) => {
...
const sampleFailuerAndDecider = (sample) => (typeRequirements) => async (commandEventTypes) => {
...
const sampleFinalTypes = (sample) => (stateTypes) => (commandEventTypes) => async (systemTypes) => {
