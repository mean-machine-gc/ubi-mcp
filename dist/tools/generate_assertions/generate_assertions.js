...
import { z } from 'zod';
export const addGenerateAssertions = (mcp) => (session) => {
    mcp.addTool({
        ...
        parameters: z.object({
            ...
            validation_focus: z.array(z.union([
        ...
        execute: async (args) => {
            const { lifecycle_yaml, validation_focus = ["completeness", "security", "business_rules"], iteration_depth = 3 } = args;
...
const sampleAnalysis = (sample) => async (lifecycle_yaml) => {
...
const sampleBaseAssertions = (sample) => async (structureAnalysis) => {
...
const sampleByFocusArea = (sample) => (validation_focus) => (baseAssertions) => async (lifecycle_yaml) => {
    const securityEnhanced = validation_focus.includes("security") ? await sample(`Enhance these assertions with security considerations:
      ...
      Integrate security enhancements into the existing structure.`) : baseAssertions;
    const validationAnalysis = await sample(`Validate these assertions for completeness and consistency:
    ...
    const finalAssertions = await sample(`Apply validation feedback to create the final assertions.yaml:
