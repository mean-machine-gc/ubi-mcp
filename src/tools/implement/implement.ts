import { Context, FastMCP, FastMCPSession } from "fastmcp";
import { sampleLlm } from "../../utils/utils";
import { z } from 'zod';
import { ubiTypes } from "../../utils/ubi/ubi-types";


export const addImplement = (mcp: FastMCP): void => {
    mcp.addTool(
          {
...
const sampleDomainModel = 
    (sample: (userPrompt: string) => Promise<unknown>) => 
    async (lifecycle_yaml: string) => {
