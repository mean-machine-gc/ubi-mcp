...
export const addGenerateTests = (mcp: FastMCP) => (session: FastMCPSession): void => {
    mcp.addTool(
          {
            name: "generate_tests",
...
const sampleFinalTests = 
    (sample: (userPrompt: string) => Promise<unknown>) => 
    (testRequirements: any) =>
    (sharedHelpers: any) =>
    (operationTests: any) =>
    async (edgeCaseTests: any) => {
